...
check_flag() {
    ...
    # ./s21_grep $flag_1 "$input" $flag $test_file > output_prog.txt 2>&1
    ./s21_grep $flag_1 $test_pattern $flag "$input" $test_file > output_prog.txt 2>&1

    # Запуск оригинального grep с флагом и запись вывода в временный файл
    # grep $flag_1 "$input" $flag $test_file > output_grep.txt 2>&1
    grep $flag_1 $test_pattern $flag "$input" $test_file > output_grep.txt 2>&1

    if diff output_prog.txt output_grep.txt > /dev/null; then
        echo "Флаг '$flag_1' '$flag': OK"
    else
        echo "Флаг '$flag_1' '$flag': Ошибка"
        echo "Различия:"
